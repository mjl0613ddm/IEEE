...
def aggregate_model_results(model_name: str, results: List[Dict]) -> Dict:
    ...
    if metadata_list:
        summary['metadata'] = {
            ...
            'metric_name': metadata_list[0].get('metric_name', 'unknown'),
        }
...
def main():
    ...
    all_results = discover_risk_feature_results(risk_feature_root)
    ...
    for model_name, results in sorted(model_results.items()):
        ...
        # 保存单个模型的汇总JSON
        model_output_dir = risk_feature_root / model_name
    ...
    create_all_models_comparison(model_summaries, risk_feature_root)
    print()
    ...
    print("汇总完成！")
    print("=" * 60)
    print(f"模型汇总保存在: {risk_feature_root}/{{model}}/{{model}}_summary.json/csv")
    print(f"全模型对比保存在: {risk_feature_root}/all_models_summary.json/csv")
    print("=" * 60)
