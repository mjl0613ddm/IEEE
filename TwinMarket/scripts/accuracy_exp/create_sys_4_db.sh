...
# 从 sys_1000.db 或 sys_100.db 中提取4个用户创建 sys_4.db
# 支持随机选择或顺序选择
...
# 检查可用的源数据库
if [ -f "data/sys_1000.db" ]; then
    SOURCE_DB="data/sys_1000.db"
elif [ -f "data/sys_100.db" ]; then
    SOURCE_DB="data/sys_100.db"
else
    echo "❌ 找不到源数据库: data/sys_1000.db 或 data/sys_100.db"
    exit 1
fi

TARGET_DB="data/sys_4.db"
NUM_USERS=4
...
-- 复制 TradingDetails 表（对应选择的4个用户的交易记录）
