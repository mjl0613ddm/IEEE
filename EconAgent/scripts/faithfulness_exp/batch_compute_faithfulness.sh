...
# 模型名称列表（例如: gpt, claude, qwen等）
# 如果不设置，将自动查找所有模型目录（排除_rm后缀）
MODEL_NAMES=()
...
# 示例: METRIC_TYPES=("deletion_top_1" "deletion_top_3" "deletion_top_5" "deletion_top_10" "deletion_top_20" "insertion_top_5")
...
SUMMARY_SCRIPT="${BASE_PATH}/scripts/faithfulness_exp/generate_faithfulness_summary.py"

# 数据目录和结果目录
DATA_DIR="${BASE_PATH}/datas"
RESULTS_DIR="${BASE_PATH}/results/faithfulness_exp"
...
process_faithfulness_method_metric() {
    ...
    local method="$3"
    local metric_type="$4"
    local model_path="${model_name}/${subfolder}"
    local data_path="${DATA_DIR}/${model_path}"
    
    log "      - 处理 ${method} 方法 / ${metric_type} 指标..."
    
    # 检查数据目录是否存在
    if [ ! -d "$data_path" ]; then
        error "        数据目录不存在: $data_path"
        return 1
    fi
    ...
    case "$method" in
        "shapley")
            if [ ! -d "${data_path}/shapley" ]; then
                error "        Shapley目录不存在: ${data_path}/shapley，跳过"
                return 1
            fi
            ;;
        "random"|"loo"|"llm"|"mast")
            if [ ! -d "${data_path}/faithfulness_exp/${method}" ]; then
                error "        ${method}方法的数据目录不存在: ${data_path}/faithfulness_exp/${method}，跳过（需要先运行compute_*_baseline.py）"
                return 1
            fi
            ;;
        *)
    ...
    # 检查结果文件是否已存在
    result_file="${RESULTS_DIR}/${model_name}/${subfolder}/faithfulness_results_${method}_${metric_type}.json"
    if [ -f "$result_file" ]; then
        log "        结果文件已存在，跳过: $result_file"
        return 0
    fi
...
process_subfolder() {
    ...
    log "完成: ${model_path} (成功: ${success_count}, 失败: ${fail_count})"
...
process_model() {
    ...
    if python3 "$SUMMARY_SCRIPT" "$model_name" >> "$LOG_FILE" 2>&1; then
        log "  汇总文件生成成功"
    else
        error "  汇总文件生成失败"
    fi
