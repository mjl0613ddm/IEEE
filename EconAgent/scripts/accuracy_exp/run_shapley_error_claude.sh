#!/bin/bash
# 计算完全Shapley值和蒙特卡罗方法的误差 - GPT-4o模型
# 验证蒙特卡罗方法的近似准确性
...
# 数据路径（相对于项目根目录）
REAL_ACTIONS_JSON="data/claude-verify/actions_json/all_actions.json"
...
# 随机种子（用于多次运行取平均）
SEEDS=(42 43 401 613 46 47 48 49 716 51)  # 10个种子

# MC采样次数列表（用于测试不同采样次数的误差）
MC_SAMPLES_LIST=(10 100 1000 10000)

# 输出目录
OUTPUT_DIR="results/claude_shapley_error_analysis"
...
N_JOBS=64  # 并行进程数，用于加速完全Shapley计算（1=串行，>1=并行，建议设置为CPU核心数）
...
每个种子会分别计算exact和MC Shapley值，然后汇总结果。
...
注意：
  ...
  - 对于4个agent、5个时间步，需要计算2^20=1,048,576个子集
  - 每个种子会分别计算exact和MC Shapley值，确保种子匹配
...
echo "  - ⚠️  重要：每个种子会分别计算exact和MC Shapley值，确保种子匹配"
echo ""
echo "输出:"
...
if [[ $EXIT_CODE -eq 0 ]]; then
    ...
    echo ""
    echo "输出文件："
    echo "  - exact_shapley_seed*.npy: 每个种子的完全Shapley值"
    echo "  - exact_shapley_values_mean.npy: 所有种子的平均完全Shapley值"
    echo "  - mc_shapley_n*_seed*.npy: 每个种子、每个采样次数的MC Shapley值"
